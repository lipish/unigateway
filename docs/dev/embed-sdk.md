...
当前版本约定：默认推荐直接依赖 `unigateway-sdk`；若混用 `unigateway-core`、`unigateway-protocol`、`unigateway-host`，应保持同一 release line（例如与 `2.0.x` 对齐）。
...
- **保守兼容的部分**：`embed` feature 没有删除，只是降级为兼容别名；默认依赖路径仍然保持简单的 `unigateway-sdk = "2.0"`（或与你锁定的 minor 对齐）。
